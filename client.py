...
class GUI:
    ...
    def display_chat_box(self):
        frame = Frame()
        Label(frame, text='Chat', font=("arial", 12,"bold")).pack(side='top', padx=270)
    ...
    def display_chat_entry_box(self):   
        frame = Frame()
        Label(frame, text='Conversar em Wired', font=("arial", 12,"bold")).pack(side='top', anchor='w', padx=120)
    ...
    def on_enter_key_pressed(self, event):
        if len(self.name_widget.get()) == 0:
            messagebox.showerror("Erro: Sem username", "Você precisa de um Username para mandar mensagem!")
            return
        self.send_chat()
        self.clear_text()
    ...
    def on_close_window(self):
        if messagebox.askokcancel("Sair", "Você quer sair mesmo?"):
            self.root.destroy()
